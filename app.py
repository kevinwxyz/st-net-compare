...
def visualize_network_with_modules(graph, module_attribute="module", selected_modules=None):
    ...
    # Add filtered nodes and edges to Pyvis network
    for node in filtered_nodes:
        data = graph.nodes[node]
        taxa_name = data.get("name", str(node))
        module_id = data.get(module_attribute, "Unknown")
...
if uploaded_file_1 and uploaded_file_2:
    ...
    # Interactive module-specific visualizations
    def get_node_module_mapping(communities, graph):
        module_dict = {}
        for idx, community in enumerate(communities):
            for node in community:
                module_dict[node] = idx + 1
        nx.set_node_attributes(graph, module_dict, "module")
        return module_dict

    st.header("Interactive Module Visualizations")

    for idx, (graph, communities, label) in enumerate(zip([G1, G2], [communities1, communities2], ["Network 1", "Network 2"])):
        module_mapping = get_node_module_mapping(communities, graph)
        st.subheader(f"{label} Module Statistics and Visualization")

        module_metrics = []
        for module_id, nodes in module_mapping.items():
            subgraph = graph.subgraph([n for n in graph.nodes if graph.nodes[n]["module"] == module_id])
        ...
        # Multiselect for filtering modules
        selected_modules = st.multiselect(f"Select modules to view for {label}", options=module_df["Module ID"].tolist())

        if selected_modules:
            html_file = visualize_network_with_modules(graph, module_attribute="module", selected_modules=selected_modules)
            st.components.v1.html(open(html_file, "r").read(), height=750)
        else:
            st.write("Select one or more modules to visualize.")
